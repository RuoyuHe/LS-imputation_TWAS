...
white_unrelated_keep = fread('~/deepRIV/UKB/data/white_unrelated_keep_ind.txt',
                             header = F)
ukb_pheno_all = fread('/home/panwei/shared/LSimputedHDLtrait/original_hdl.csv')
...
stage1 <- function(gene_ind, 
  ...
  #                                          conditionMessage(e),
  #                                          "\n")})
  # remove_command = paste0("rm ~/deepRIV/UKB/data/ForUKB/ref_",
  #                         gene_name,".*")
  # system(remove_command)

  ukb_out = paste0('~/deepRIV/UKB/data/ForUKB/',gene_name)
  ukb_plink_command = paste0('module load plink/2.00-alpha-091019; plink2 --pfile vzs ',prefix,
                             ' --chr ', chr,
                             ' --extract ','~/deepRIV/UKB/data/ForUKB/',gene_name,'_rs.txt',
                             ' --maf 0.05 --hwe 0.001  ',
                             ' --make-bed',
