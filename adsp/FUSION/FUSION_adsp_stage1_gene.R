...
if(tissue=='Whole_Blood'){
    end_ind = min(file_num*runs, 19626)
}else if(tissue=='Brain_Hippocampus'){
    end_ind = min(file_num*runs , 23648)
}
...
gene_exp = na.omit(gene_exp)[,c(1:4)]

############### missing genes (comment out when not running for missing genes) ###############
# results_folder = paste0("/home/panwei/he000176/deepRIV/ImputedTraits/results/adsp/gene/",tissue,"/stage1/")
# files_in_folder = list.files(path = results_folder, pattern = "\\.RData$", full.names = FALSE)
# genes_in_folder = sub("\\.RData$", "", files_in_folder)  
# missing_idx = which(!(gene_exp$gene_id %in% genes_in_folder))
# end_ind = min(file_num*runs , length(missing_idx))

# for(gene_ind in missing_idx[start_ind:end_ind]){
for(gene_ind in start_ind:end_ind){
    
    gene_name = as.character(gene_exp$gene_id[gene_ind])
    start = max(0, floor(gene_exp$start[gene_ind]/1000) - 500)
    end = ceiling(gene_exp$end[gene_ind]/1000) + 500
    chr = gene_exp$chr[gene_ind]

    gtex_exp = fread(paste0('/scratch.global/he000176/GTEx/expression_adj_cov/',tissue,'/',gene_name,'.txt'))

    data_save_path = paste0('/scratch.global/he000176/ImputedTraits/stage1_data/',tissue,'/',gene_name,'.rds')
    results_path = paste0('/home/panwei/he000176/deepRIV/ImputedTraits/results/adsp/gene/',tissue,'/stage1/')
    ...
    GTEx_dir = create_random_directory('/scratch.global/he000176/GTEx/tmp/')
    ...
    plink_command = paste0("plink2 --bfile /home/panwei/he000176/deepRIV/UKB/data/GTEx/GTEx_Analysis_2017-06-05_v8_WholeGenomeSeq_838Indiv_Analysis_Freeze.SHAPEIT2_phased_MAF01",
                         " --chr ",chr," --from-kb ",start," --to-kb ",end,
                         " --geno 0 --maf 0.05 --hwe 1e-15 midp  ",
    ...
    adsp_plink_command = paste0('plink2 --bfile ',
                              adsp_snp_path,
                             ' --chr ', chr,
    ...
    keep_ukb_indiv = intersect(ukb_igap$f.eid,ukb_snp$fam$IID)
    ...
    ukb_igap = ukb_igap[match(rownames(ukb_snp_bed),ukb_igap$f.eid),,drop=FALSE]
    ...
    tmp_idx = match(gtex_exp$IID, rownames(SNP_BED))
    SNP_BED = SNP_BED[tmp_idx,]
    rm(tmp_idx)

    res = cv_stage1(SNP_BED, gtex_exp, method='both')

    # for TWAS-LQ
    gtex_exp2 = gtex_exp
    gtex_exp2$pheno = gtex_exp2$pheno^2
    res_x2 = cv_stage1(SNP_BED, gtex_exp2, method='both')

    ### UKB
    # y_ukb_ID = as.numeric(ukb_pheno$f.eid)
    y_ukb_ID = as.integer(rownames(ukb_snp_bed))
    ...
    ukb_snp_bed_reduced = ukb_snp_bed[,nonzero_idx,drop=F]
    ...
    if (TRUE){
        save(res, res_x2, num_snps_gtex, num_snps_common, rs_id, gene_name, chr, file = paste0(results_path,'/',gene_name,'.RData'))
    }
